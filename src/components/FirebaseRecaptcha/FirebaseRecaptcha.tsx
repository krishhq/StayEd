...
const DEFAULT_WEB_APP_OPTIONS = undefined;
...
function getWebviewSource(firebaseConfig: any, firebaseVersion: string, appVerificationDisabledForTesting = false, languageCode?: string, invisible?: boolean) {
    const version = firebaseVersion || '8.0.0';
    return {
...
function validateFirebaseConfig(firebaseConfig: any) {
    if (!firebaseConfig) {
        throw new CodedError('ERR_FIREBASE_RECAPTCHA_CONFIG', `Missing firebase web configuration. Please pass the "firebaseConfig" prop.`);
    }
...
export default function FirebaseRecaptcha(props: FirebaseRecaptchaProps) {
    ...
    if (!firebaseConfig) {
        console.error(`FirebaseRecaptcha: Missing firebase web configuration.`);
        return null;
    }
    ...
    return (
        <WebView
            ...
            source={getWebviewSource(firebaseConfig, firebaseVersion || '8.0.0', appVerificationDisabledForTesting, languageCode, invisible)}
            onError={onError}
            onMessage={(event: any) => {
